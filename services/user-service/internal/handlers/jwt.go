...
func GetUserFromContext(c *gin.Context) (userID string, fullName string, email string, userType string, isVerified bool, ok bool) {
	...
	fullNameVal, exists := c.Get("full_name")
	if !exists {
		return userID, "", "", "", false, false
	}
	fullName, ok = fullNameVal.(string)
	if !ok {
		return userID, "", "", "", false, false
	}
	...
	if !exists {
		return userID, fullName, "", "", false, false
	}
	...
	if !exists {
		return userID, fullName, email, "", false, false
	}
	...
	if !exists {
		return userID, fullName, email, userType, false, false
	}
	...
	return userID, fullName, email, userType, isVerified, true
}
