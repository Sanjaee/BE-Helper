...
func (h *Hub) Run() {
	for {
		...
		case client := <-h.register:
			...
			h.mu.Unlock()
			log.Printf("✅ Location WebSocket client registered for order: %s (total: %d)", client.orderID, len(h.clients[client.orderID]))
		...
		case client := <-h.unregister:
			...
			if clients, ok := h.clients[client.orderID]; ok {
				if _, ok := clients[client]; ok {
					...
					log.Printf("❌ Location WebSocket client unregistered for order: %s", client.orderID)
				}
			}
			h.mu.Unlock()
			...
			if len(clients) > 0 {
				...
				for client := range clients {
					...
					case client.send <- jsonData:
						log.Printf("📤 Location broadcast sent to client for order: %s", orderIDStr)
					default:
